...
export const CHANNEL = "PIEPIO_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "piepio_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "piepio_focused_element" as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _piepioLastMsg) return; // identical – drop
    _piepioLastMsg = key;
  } catch {
...
// Helper to parse data-piepio-id to extract file path, line, and column
const parsePiepioId = (
  piepioId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = piepioId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-piepio-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-piepio-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the piepio ID from the element to ensure we're working with the right one
    const piepioId = element.getAttribute("data-piepio-id");
    if (!piepioId) return;
    ...
    if (newText !== oldText) {
      const parsed = parsePiepioId(piepioId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same piepio ID
    const allMatchingElements = document.querySelectorAll(
      `[data-piepio-id="${piepioId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(piepioId) || {};
    appliedStylesRef.current.set(piepioId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(piepioId);
  ...
  const flushImageSrcChange = () => {
    ...
    const piepioId = imgElement.getAttribute("data-piepio-id");
    if (!piepioId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "PIEPIO_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same piepio ID
        const allMatchingElements = document.querySelectorAll(
          `[data-piepio-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "PIEPIO_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-piepio-id
        const piepioId = element.getAttribute("data-piepio-id");
        if (piepioId) {
          const parsed = parsePiepioId(piepioId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-piepio-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-piepio-id");
        ...
        const tagName =
          hit.getAttribute("data-piepio-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same piepio ID
        const allMatchingElements = document.querySelectorAll(
          `[data-piepio-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-piepio-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-piepio-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same piepio ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-piepio-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-piepio-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same piepio ID
        const allMatchingElements = document.querySelectorAll(
          `[data-piepio-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-piepio-name") ||
